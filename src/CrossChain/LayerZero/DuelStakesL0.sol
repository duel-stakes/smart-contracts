...
contract duelStakesL0 is OApp,Pausable{
    ...
    constructor (address __paymentToken,address __treasuryAccount,address __operationManager, address _endpoint,
    address _owner) OApp(_endpoint, _owner) Ownable(_owner) Pausable(){
    ...
    function getUserDeposits(string memory _title, uint256 _timestamp,uint256 chainId,address _user) public view returns(uint256,uint256,uint256){
        bytes32 _id = keccak256(abi.encode(_timestamp,_title));
        return (duels[_id].userDeposits[_user][chainId]._amountOp1,duels[_id].userDeposits[_user][chainId]._amountOp2,duels[_id].userDeposits[_user][chainId]._amountOp3);
    }
    ...
    function _depositPick(uint256 _amount, pickOpts _pick, address sender,betDuel storage _duel, uint256 chainId) internal{
    ...
    function _updateClaim(betDuel storage _duel, address _claimer) internal returns (uint256 _payment) {
        if(_duel.releaseReward == pickOpts.opt1){
            _payment = (_duel.userDeposits[_claimer][block.chainid]._amountOp1 * _duel.totalPrizePool) / _duel.opt1PrizePool;
            ...
            _payment = (_duel.userDeposits[_claimer][block.chainid]._amountOp2 * _duel.totalPrizePool) / _duel.opt2PrizePool;
            _duel.userClaimed[_claimer] = true;
            _duel.unclaimedPrizePool -= _payment;
            _transferUserAmount(_payment);
