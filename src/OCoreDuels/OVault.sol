...
//     mapping (bytes32 => bool) public duels;
...
//     event emergencyWithdrawal(uint256 indexed _amount);
//     event duelSent(string indexed _title, uint256 indexed _eventDate, uint256 indexed _openAmount);
...
//         if(duels[keccak256(abi.encode(_eventDate,_title))])
...
//         bool success = _paymentToken.transfer(_treasuryAccount, _amount);
//         if(!success)
...
//         _sendCreateDuel(_newDuel);
//         emit duelSent(_newDuel.duelTitle, _newDuel.eventTimestamp, _newDuel.initialPrizePool);
//     }
...
//         _sendDeposit(_amount,_option,msg.sender,_title,_eventDate);
